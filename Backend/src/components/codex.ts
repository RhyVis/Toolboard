...
function codexTrad(text: string, dec: boolean): string {
  return dec ? cnchar.convert.tradToSimple(text) : cnchar.convert.simpleToTrad(text);
}

// 4: Mars lang? chinese strange char replace

function codexSpark(text: string, dec: boolean): string {
  return dec ? cnchar.convert.sparkToSimple(text) : cnchar.convert.simpleToSpark(text);
}
